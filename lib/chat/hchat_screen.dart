...
import 'package:resq/chat/profile_page.dart';
import 'package:resq/chat/search_page.dart';
import 'package:resq/services/auth_services.dart';
import 'package:resq/services/database_services.dart';
import 'package:resq/widgets/group_tile.dart';
  ...
  groupList() {
    return StreamBuilder(
      ...
      builder: (context, AsyncSnapshot snapshot) {
        ...
        if (snapshot.hasData) {
          final data = snapshot.data.data(); // Get the data from the DocumentSnapshot

          if (data != null && data['groups'] != null) {
            final groupsList = data['groups'];

            if (groupsList.length != 0) {
              return ListView.builder(
                itemCount: groupsList.length,
                itemBuilder: (context, index) {
                  int reverseIndex = groupsList.length - index - 1;
                  return GroupTile(
                    groupId: getId(groupsList[reverseIndex]),
                    groupName: getName(groupsList[reverseIndex]),
                    userName: userName,
                  );
                },
              );
